...
export const AllProducts = ({ BannerImg }) => {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:5000/api/products") // Ensure backend API is correct
      .then((res) => {
        setAllProducts(res.data); // Set all products directly from response
      })
      .catch((err) => {
  ...
  return (
    <>
      {/* Banner Image */}
      {BannerImg && <img src={BannerImg} alt="All Products Banner" className="w-full h-auto" />}
