...
export const KidsProducts = ({ BannerImg }) => {
  const { addToCart } = useCart();
  const navigate = useNavigate();
  const [kidsProducts, setKidsProducts] = useState([]);

  // Fetching Men's Products from the Backend
  useEffect(() => {
    axios
      .get("http://localhost:5000/api/products") // Adjust backend URL if needed
      .then((res) => {
        const filteredProducts = res.data.filter(
          (product) => product.category === "kidsProducts"
        );
  ...
  return (
    <>
      {/* Banner Image */}
      <img src={BannerImg} alt="Men Banner" className="w-full h-auto" />
